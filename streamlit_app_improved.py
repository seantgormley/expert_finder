...
Improved Streamlit UI for the Expert Finder application.
"""
...
from expert_finder_improved import find_and_rank, extract_company_name, check_api_keys
...
if submitted and query:
    ...
    with st.spinner(f"Searching for experts related to {company_name}..."):
        try:
            ...
            st.info("Please set up your API keys in the .env file or as environment variables.")
        except Exception as e:
            st.error(f"An error occurred: {str(e)}")
            st.info("If this error persists, please check your API keys and internet connection.")
...
with st.sidebar:
    ...
    if cb_key and cb_key != "YOUR_CRUNCHBASE_KEY":
        st.success("✅ Crunchbase API Key: Set")
    else:
        st.error("❌ Crunchbase API Key: Not set")
        
    if pdl_key and pdl_key != "YOUR_PDL_KEY":
        st.success("✅ People Data Labs API Key: Set")
    else:
        st.error("❌ People Data Labs API Key: Not set")
